...
def find_best_split(rows):
    ...
    for col in range(n_features):  # for each feature
        ...
        for val in values:  # for each value
            ...
            #print("true rows :",len(true_rows))
            #print("false rows :",len(true_rows))
...
k=0
k1=0
def print_three(node,dot):
    print("print_three")
    dot.node('1',str(node.question))
    dot.node('2',str(node.true_branch.question))
    dot.node('3',str(node.false_branch.question))
    dot.edge('1','2',label="true")
    dot.edge('1','3',label="false")
...
def load_csv_to_header_data(filename):
    ...
    data = {
        'header': headers,
        'rows': all_row[1:],      #stores only the data rows now
    }
    return data
...
if __name__ == '__main__':
    ...
    training_data = list(data['rows'])
    header = list(data['header'])

    print("training")
    my_tree = build_tree(training_data)

    dot = Digraph('decisionz_tree')
